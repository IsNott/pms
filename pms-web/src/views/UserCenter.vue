<template>
    <div>
        <el-row>
            ...
            <el-col :span="10">
                <el-form :label-position="labelPosition" :rules="rules" label-width="80px" ref="detail" :model="detail">
                    <el-form-item label="昵称：" prop="nickName">
                        <el-input class="detail" v-model="detail.nickName" placeholder="请输入昵称"></el-input>
                    </el-form-item>
                    <el-form-item label="邮箱：" prop="email">
                        <el-input class="detail" v-model="detail.email" placeholder="请输入邮箱"></el-input>
                    </el-form-item>
                    <el-form-item label="签名：" prop="token">
                        <el-input type="textarea" class="detail" v-model="detail.token" placeholder="请输入签名"></el-input>
                    </el-form-item>
                    <el-form-item>
                        <el-button class="reDetail" @click="updateUserInfo('detail')">修改信息</el-button>
                        <el-button class="reDetail" @click="reset('detail')">清空</el-button>
                    </el-form-item>
                </el-form>
            </el-col>
        </el-row>
    ...
    export default {
        ...
        data() {
            return {
                ...
                userInfo: JSON.parse(window.sessionStorage.getItem("userInfo")) ? JSON.parse(window.sessionStorage.getItem("userInfo")) : {},
                rules: {
                    nickName: [
        ...
        methods: {
            ...
            // vaild(detail) {
            //     if (detail.nickName == '' || detail.nickName == null) {
            //         return false;
            //     }
            // },
            isSee() {
                ...
                this.postRequest("/pms/user/getInfoById", {id}).then(res => {
                    if (res.code == 0) {
                        ...
                        this.detail.token = info.token
                        this.detail.nickName = info.nickName
            ...
            updateUserInfo(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        var user = this.detail;
                        this.postRequest("/pms/user/user/userupdate", user).then(res => {
                            if (res.code == 0) {
            ...
            confirm(type) {
                this.$refs.cropper.getCropData(res => {
                    var data = res;
                    this.dialogVisible = false;
                    this.postRequest("/pms/user/upload", {data}).then(res => {
                        if (res.code == 0) {
                            this.$message.success(res.msg);
                            // 刷新本页面
                            this.$router.go(0)
